...
class SnatchFreeleechCommand extends Command {
  ...
  downloadTorrent(item, callback) {
    ...
    this.request
      ...
      .pipe(fs.createWriteStream(`${this.torrentDir}/${item.torrentId}.torrent.part`))
      .on('finish', err => {
        fs.renameSync(`${this.torrentDir}/${item.torrentId}.torrent.part`, `${this.torrentDir}/${filename}`);
